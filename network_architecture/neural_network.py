...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        C_down_1 = torch.ones([1, 1, 6, 12, 12])
        C_down_2 = torch.ones([1, 1, 12, 24, 24])
        C_down_3 = torch.ones([1, 1, 24, 48, 48])
        C_down_4 = torch.ones([1, 1, 48, 96, 96])
        C_down_5 = torch.ones([1, 1, 48, 192, 192])
        C_down_6 = torch.ones([1, 1, 48, 192, 192])

        training_dict_end = {'C_1': 2, 'C_2': 4, 'C_3': 6, 'C_4': 8, 'C_5': 10}
        ...
        for m in range(mirror_idx):
            if m == 0:
                pred = self.inference_apply_nonlin(self(x, C_down_1, C_down_2, C_down_3, C_down_4, C_down_5))
                result_torch += 1 / num_results * pred

            if m == 1 and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, )), C_down_1, C_down_2, C_down_3, C_down_4, C_down_5))
                result_torch += 1 / num_results * torch.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (3, )), C_down_1, C_down_2, C_down_3, C_down_4, C_down_5))
                result_torch += 1 / num_results * torch.flip(pred, (3,))

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3)), C_down_1, C_down_2, C_down_3, C_down_4, C_down_5))
                result_torch += 1 / num_results * torch.flip(pred, (4, 3))

            if m == 4 and (0 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (2, )), C_down_1, C_down_2, C_down_3, C_down_4, C_down_5))
                result_torch += 1 / num_results * torch.flip(pred, (2,))

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2)), C_down_1, C_down_2, C_down_3, C_down_4, C_down_5))
                result_torch += 1 / num_results * torch.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2)), C_down_1, C_down_2, C_down_3, C_down_4, C_down_5))
                result_torch += 1 / num_results * torch.flip(pred, (3, 2))

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2)), C_down_1, C_down_2, C_down_3, C_down_4, C_down_5))
